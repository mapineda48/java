const urlCategory = "/api/Category/all";
let categorys = "";
const amountCategorys = 20;
const baseUrl = "/api/Ortopedic/";
let _onunmoun = null;
const body = $("body");
const main = $("#state");
...
body.find("#btn-create").on("click", (event) => {
...
function fetchCategorys(onSuccess) {
    loading();
    $.ajax({
        url: urlCategory,
        dataType: "JSON",
        ...
        success(data) {
            if (categorys)
                return;
            if (!data.length) {
                message("Por favor primero agregue categorias");
                return;
            }
            if (data.length > amountCategorys) {
                data.length = amountCategorys;
            }
            categorys = data.map((val) => `<option value="${val.id}">${val.name}</option>`);
            onSuccess();
            fetchRecords();
        },
        error(jqXHR, status, err) {
...
function printRecords(res) {
    ...
    res.forEach((res) => {
        const record = render(tRecord)
            ...
            .set("brand", res.brand)
            .set("year", res.year)
            .set("category", res.category.name)
            .get();
